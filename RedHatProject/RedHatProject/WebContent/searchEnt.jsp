...
<div class="wrapper">
	...
	<div class="entinfo-contents-container">
	  ...
	  <script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=d995312857d2824fed7e37880052f036&libraries=services,clusterer,drawing"></script>
	  <script>
      ...
      %>
		         <form method="get" action="enterprise_evaluation.jsp">
		         ...
		         <div class="evaluate-button-area">
			         ...
			         <input class="button" type="button" value="새로고침" onClick="window.location.reload()"/>
		         </div>
		      </form>
...
<div class="entinfo-comment-area">  
<form method="post" action="sns_control.jsp?action=newmsg">
<span class="entinfo-comment-area-input">
	<b>댓글달기&nbsp;</b>
		<input type="hidden" name="e_code" value="<%=code%>" /> 
		<input type="hidden" name="uid" value="<%=uid%>"/>
		<input class="comment-text" type="text" size=50  name="reply_content"/>
		<input class="button" type="submit" value="등록"/>
		<input class="button" type="button" value="새로고침" onClick="window.location.reload()"/>
		
</span>
</form>
	<h5><%=edao.findEnterpriseName(code) %>의 모든 댓글</h5>
    ...
    <tr>
	<%
		for(int i=0; i<msgDAO.getReplyMessage(code).size(); i++){
			if(msgDAO.getReplyMessage(code).size()==0){
	...
	%>
	   <tr>
	   		<td><%=i+1 %></td>
			<td><%= msgDAO.getReplyMessage(code).get(i).getMessage().getUid() %></td>
			<td><%= msgDAO.getReplyMessage(code).get(i).getMessage().getReply_content() %></td>
			<td><%= msgDAO.getReplyMessage(code).get(i).getMessage().getCurrent()%></td>
		</tr>
	<% 			
			}
	%>
